...
module.exports = {
  ...
  category: 'economy',
	...
	async	execute(message, args) {
		...
		/*
	let rob = db.fetch(`${message.guild.id}rob`);
	
			if (rob === '0') {
				return message.reply('robs disabled here');
			}
			*/
		...
		let cooldown = 15000; // 12 hours in ms
		var result = await db.fetch(message.author.id)
		let lastrob = parseInt(result.robcd + '000')

		if (lastrob !== null && cooldown - (Date.now() - lastrob) > 0) {
			// If user still has a cooldown
			let timeObj = cooldown - (Date.now() - lastrob);
			re(
				'Woah slow down',
				`You have to wait` + '`' + timeObj / 1000 + '`s before robbing again.'
			);
		} else {
			...
			if (parseInt(bal) < 1000) {
				...
				db.set(message.author.id, 'robcd', time);
