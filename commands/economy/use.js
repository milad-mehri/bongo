...
module.exports = {
  ...
  category: 'economy',
	...
	async execute(message, args, premiumusers) {
		...
		let lastUse = parseInt(result.usecd + '000')

		if (lastUse !== null && cooldown - (Date.now() - lastUse) > 0) {
			// If user still has a cooldown
			let timeObj = cooldown - (Date.now() - lastUse);
			return re(
				'Woah slow down',
				`You have to wait` + '`' + timeObj / 1000 + '`s before using a item again.'
			);
		} else {
			var time = Date.now().toString().slice(0, -3)
			var result = await db.set(message.author.id, 'usecd', time)
